...
lemma {x} {y} xy↓ = proj₁ ((IsPartialMonoid.assocʳ (IsReflexivePartialMonoid.isPMon X-RPMon)) {x} {y} {x ∙[ xy↓ ] y} xy↓ (IsReflexivePartialMonoid.reflexive X-RPMon))

lemmaR : ∀ {y z} → (yz↓ : y R z) → (y ∙[ yz↓ ] z) R y
lemmaR {y} {z} yz↓ = proj₁ ((IsPartialMonoid.assocˡ (IsReflexivePartialMonoid.isPMon X-RPMon)) {y ∙[ yz↓ ] z} {y} {z} yz↓ (IsReflexivePartialMonoid.reflexive X-RPMon))
...
mutual
  ...
  pow-commutes-gen zero x y xy=yx r r' = trans (iʳ r) (sym (iˡ r'))
    where
      iʳ = IsReflexivePartialMonoid.identityʳ' (IsReflexivePartialMonoid.isPMon X-RPMon)
      iˡ = IsReflexivePartialMonoid.identityˡ' (IsReflexivePartialMonoid.isPMon X-RPMon)
  ...
  pow-commutes-gen (suc (suc n)) x y xy=yx r r' =
    ...
    (powR (suc n) y ∙[ powR-R (suc n) y ] y) ∙[ r' ] x
      ∎ where
        open ≡-Reasoning
        A← = IsReflexivePartialMonoid.assocˡ (IsReflexivePartialMonoid.isPMon X-RPMon)
        A→ = IsReflexivePartialMonoid.assocʳ (IsReflexivePartialMonoid.isPMon X-RPMon)
        A→₃ = IsReflexivePartialMonoid.assocʳ₃ (IsReflexivePartialMonoid.isPMon X-RPMon)

  pow-commutes : (n : ℕ) → (x : A) → (r : x R pow n x) → (r' : powR n x R x) →
                 x ∙[ r ] pow n x ≡ powR n x ∙[ r' ] x
  pow-commutes n x r r' =
    pow-commutes-gen n x x (λ p q → cong (∙ x x) (IsReflexivePartialMonoid.R-prop (IsReflexivePartialMonoid.isPMon X-RPMon) p q)) r r'

  pow-R : (n : ℕ) → (x : A) → x R pow n x
  pow-R zero x = IsReflexivePartialMonoid.ε-compatʳ
                     (IsReflexivePartialMonoid.isPMon X-RPMon)
  pow-R (suc zero) x = IsReflexivePartialMonoid.reflexive X-RPMon
  pow-R (suc (suc n)) x = subst (x R_)
                                (sym (pow-commutes (suc n) x _ _))
                                (lemma (powR-R (suc n) x))

  powR-R : (n : ℕ) → (x : A) → powR n x R x
  powR-R zero x = IsReflexivePartialMonoid.ε-compatˡ
                    (IsReflexivePartialMonoid.isPMon X-RPMon)
  powR-R (suc zero) x = IsReflexivePartialMonoid.reflexive X-RPMon
