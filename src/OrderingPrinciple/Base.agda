...
STO≃Set-gives-OP (FORGETSTO⁻¹ , ((η , (η⁻¹ , η-inv)) , (ε , (ε⁻¹ , ε-inv)))) X X-set = _<_ , proof
    where
      ...
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.isStrictPartialOrder .IsStrictPartialOrder.isEquivalence = ≡-isEquivalence
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.isStrictPartialOrder .IsStrictPartialOrder.irrefl refl = IsStrictTotalOrder.irrefl isSTO' refl
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.isStrictPartialOrder .IsStrictPartialOrder.trans i<j j<k = IsStrictTotalOrder.trans isSTO' i<j j<k
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.isStrictPartialOrder .IsStrictPartialOrder.<-resp-≈ .proj₁ refl x<y = x<y
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.isStrictPartialOrder .IsStrictPartialOrder.<-resp-≈ .proj₂ refl x<y = x<y
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.compare x y with IsStrictTotalOrder.compare isSTO' (transform ε⁻¹ XX x) (transform ε⁻¹ XX y)
      ...
      ... | tri> ¬x<y ¬x=y' y<x = tri> ¬x<y (λ x=y → ¬x=y' (cong (transform ε⁻¹ XX) x=y)) y<x
      proof .IsPropStrictTotalOrder.≈-prop = X-set
      proof .IsPropStrictTotalOrder.<-prop = IsPropStrictTotalOrder.<-prop proof'
...
STO≃Set-gives-OP' (FORGETSTO⁻¹ , ((η , (η⁻¹ , η-inv)) , (ε , (ε⁻¹ , ε-inv)))) X = _<_ , proof
    where
      ...
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.isStrictPartialOrder .IsStrictPartialOrder.<-resp-≈ .proj₁ refl x<y = x<y
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.isStrictPartialOrder .IsStrictPartialOrder.<-resp-≈ .proj₂ refl x<y = x<y
      proof .IsPropStrictTotalOrder.isSTO .IsStrictTotalOrder.compare x y with IsStrictTotalOrder.compare isSTO' (transform ε⁻¹ X x) (transform ε⁻¹ X y)
      ...
      ... | tri> ¬x<y ¬x=y' y<x = tri> ¬x<y (λ x=y → ¬x=y' (cong (transform ε⁻¹ X) x=y)) y<x
      proof .IsPropStrictTotalOrder.≈-prop {x} {y} p q = lemma (IsPropStrictTotalOrder.≈-prop proof' (cong (transform ε⁻¹ X) p) (cong (transform ε⁻¹ X) q))
        where
